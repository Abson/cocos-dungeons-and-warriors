...
// const URL = 'texture/player/blockfront';

export default class BlockFontState extends States implements IState {
    constructor(private fsm: States, private title: string) {
        super();
    }

    async init() {
        const loop = AnimationClip.WrapMode.Normal;
        const URL = `texture/player/block${this.title}`;
